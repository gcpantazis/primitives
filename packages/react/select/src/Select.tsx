...
import { composeEventHandlers, getActiveElement } from '@radix-ui/primitive';
...
const SelectContentImpl = React.forwardRef<SelectContentImplElement, SelectContentImplProps>(
  (props: ScopedProps<SelectContentImplProps>, forwardedRef) => {
    ...
    const [searchRef, handleTypeaheadSearch] = useTypeaheadSearch((search) => {
      const enabledItems = getItems().filter((item) => !item.disabled);
      const currentItem = enabledItems.find((item) => item.ref.current === getActiveElement());
      const nextItem = findNextItem(enabledItems, search, currentItem);
      if (nextItem) {
        /**
...
>((props: ScopedProps<SelectScrollButtonImplProps>, forwardedRef) => {
  ...
  useLayoutEffect(() => {
    const activeItem = getItems().find((item) => item.ref.current === getActiveElement());
